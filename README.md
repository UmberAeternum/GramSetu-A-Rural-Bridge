...
- Adds a lightweight **4D-style village scene**: camera-parallax movement, firefly/harvest particles, monsoon rainfall, a moving light sweep, and a cinematic portal transition into the command centre. Both supplied village photos are part of an automatic and manually controllable live photo journey; the GramSetu team logo is rendered as a 3D hero crest and identity seal. These effects work locally in the browser and add no external API cost.
